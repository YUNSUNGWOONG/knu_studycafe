...
class delay(tk.Frame):
    def __init__(self, parent):
        ...
        self.show_image("img/시간확인_연장.png")
        ...
        def ref_password_entry_click(event):
            if self.ref_password.get() == "비밀번호를 입력하세요.":
                self.ref_passwordl.delete(0, tk.END)
        ...
        self.ref_password.place(x=307, y=433, width=364, height=41)
    ...
    def canvas_click(self, event):
        ...
        if 303 <= event.x and event.x <= 673 and 498 <= event.y <= 560:
            ...
            if self.check_membership():
                ...
                app.user_state = user_state(app)
                app.show_frame("user_state")
            else:
